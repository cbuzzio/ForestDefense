...
//				Mathf.Lerp (this.transform.position.y, target.transform.position.y, Time.deltaTime * 1.5f), -1f);
			...
			body.velocity = new Vector2 (dir.x, dir.y).normalized * 2f;
		}
	}
	...
	void OnTriggerEnter2D(Collider2D coll){
		...
		if (coll.tag == "Enemy") {
			if (tipoProy == ITipoProyectil.Simple) {
				...
				Collider2D[] others = Physics2D.OverlapCircleAll (new Vector2 (coll.gameObject.transform.position.x, coll.transform.position.y), 0.5f,1<<LayerMask.NameToLayer("Shootable"));
